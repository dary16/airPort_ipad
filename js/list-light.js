...
$(function() {
    ...
    //新建
    $('#createLight').on('click', function() {
        window.location.href = 'list-light-edit.html';
    });
});
...
function getPlanListFn(serObj) {
    ...
    var html = '';
    getRequest('airPortService/busLightPlan/list', 'post', defObj, function(data) {
        ...
        if (data.length != 0) {
            data.forEach(function(item) {
                ...
                html += '<li style="width:' + setTitleArr[1].width + '">' + item.startTime + '</li>';
                html += '<li style="width:' + setTitleArr[2].width + '">' + item.endTime + '</li>';
                if (item.status == '未完成') {
                    html += '<li class="red" style="width:' + setTitleArr[3].width + '">' + item.status + '</li>';
                } else {
                    html += '<li class="green" style="width:' + setTitleArr[3].width + '">' + item.status + '</li>';
                }
        ...
        $('.editBtn').each(function(i, v) {
            $(v).on('click', function() {
                if ($(v).attr('isEdit') == '完成') {
                    ...
                    window.location.href = 'list-light-edit.html?edit=true&id=' + data[i].id;
                }
            });
        });
        ...
        $('.goToView').each(function(i, v) {
            $(v)
                ...
                .not('.operationBtns')
                .on('click', function() {
                    window.location.href = 'list-light-edit.html?edit=true&view=true&id=' + data[i].id;
                });
        });
        ...
        $('.delBtn').each(function(i, v) {
            $(v).on('click', function() {
                layer.confirm('确定删除吗？', function(index) {
                    layer.close(index);
                    getRequest(
                        'airPortService/busLightPlan/del',
                        'post',
                        {
                            ids: data[i].id
                        },
