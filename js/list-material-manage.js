...
$(function() {
    ...
    $('#createNet').on('click', function() {
        window.location.href = 'list-material-manage-edit.html';
    });
});
...
function getPlanListFn(serObj) {
    ...
    var html = '';
    getRequest('airPortService/busBirdMaterial/list', 'post', defObj, function(data) {
        pageCount = data.records;
        data = data.rows;
        if (data.length != 0) {
            data.forEach(function(item) {
                html += '<ul class="goToView">';
                html += '<li style="width:' + setTitleArr[0].width + '">' + item.materialName + '</li>';
                html += '<li style="width:' + setTitleArr[1].width + '">' + item.materialType + '</li>';
                html += '<li style="width:' + setTitleArr[2].width + '">' + item.totalNum + '</li>';
        ...
        $('.editBtn').each(function(i, v) {
            $(v).on('click', function() {
                window.location.href = 'list-material-manage-edit.html?edit=true&id=' + data[i].materialId;
            });
        });
        ...
        $('.goToView').each(function(i, v) {
            $(v)
                ...
                .not('.operationBtns')
                .on('click', function() {
                    window.location.href = 'list-material-manage-edit.html?edit=true&view=true&id=' + data[i].materialId;
                });
        });
        ...
        $('.delBtn').each(function(i, v) {
            $(v).on('click', function() {
                layer.confirm('确定删除吗？', function(index) {
                    layer.close(index);
                    getRequest(
                        'airPortService/busBirdMaterial/del',
                        'post',
                        {
                            ids: data[i].materialId
                        },
                        function(data) {
                            layer.msg('删除成功！');
