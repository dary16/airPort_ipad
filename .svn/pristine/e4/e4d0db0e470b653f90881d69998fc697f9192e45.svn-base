...
$(function() {
    ...
    $('.searchBtn').on('click', function() {
        ...
        if (isEdit || isEdit == 'true') {
            //编辑
            getRequest('airPortService/busRoadPlan/add', 'post', editData, function(data) {
                layer.msg('编辑成功！');
                window.location.href = 'list-road.html';
            });
        } else {
            //新增
            getRequest('airPortService/busRoadPlan/add', 'post', addData, function(data) {
                layer.msg('新增成功！');
                window.location.href = 'list-road.html';
            });
        }
...
function getEditDataFn() {
    getRequest('airPortService/busRoadPlan/get', 'post', { id: curId }, function(data) {
        editData = data;
